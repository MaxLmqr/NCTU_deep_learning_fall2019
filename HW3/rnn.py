...
Original file is located at
    https://colab.research.google.com/drive/1hkgP8eR6kyQ6Hjxl6a-AWBMCJf-92ush
"""
...
model = Sequential()
model.add(SimpleRNN(128, input_shape=(X.shape[1], X.shape[2]),recurrent_initializer='glorot_uniform'))
...
# fit model
checkpoint = keras.callbacks.ModelCheckpoint('model{epoch:08d}.h5', period=10) 
results = model.fit(X, y,validation_data = (X_val,y_val),callbacks=[checkpoint], epochs=50, batch_size=8192)
...
def generate_seq(model, vocab_to_int,int_to_vocab, seq_length, seed_text, n_chars):
  ...
  for i in range(n_chars):
    ...
    yhat = np.random.choice(len(list(yhat[0])), p=yhat[0])
    out_char = int_to_vocab[yhat]
    in_text += out_char
  return in_text

text = generate_seq(model,vocab_to_int,int_to_vocab,15,'JULIET',200)
print(text)
